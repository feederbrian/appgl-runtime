...
struct GLTextureImageLevel {
    ...
    // Raw source-layout bytes for texture-view class reinterpretation.
    // Unlike nativeData, these preserve the upload byte pattern before
    // format conversion, but only when the source pixel width matches the
    // GL texture-view class byte width.
    std::vector<std::uint8_t> rawUploadData;
    std::size_t rawUploadBpp = 0;
...
struct GLRenderbufferObject {
    ...
    bool colorShadowAuthoritative = false;
    std::vector<std::uint8_t> colorShadowCoverage;
    std::size_t colorShadowCoveredPixels = 0;
...
struct GLProgramObject {
    ...
    // Hidden uniforms synthesized for shader draw parameters on Metal paths.
    GLint shaderDrawIDUniformLocation = -1;
    GLint shaderBaseVertexUniformLocation = -1;
    GLint shaderBaseInstanceUniformLocation = -1;
