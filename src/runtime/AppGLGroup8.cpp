...
static void APIENTRY glCopyTexSubImage3D(GLenum target, GLint level, GLint xoffset, GLint yoffset, GLint zoffset, GLint x, GLint y, GLsizei width, GLsizei height) {
    ...
    (void)ctx->copyTexSubImage3D(target, level,
...
static void APIENTRY glGetUniformIndices(GLuint program, GLsizei uniformCount, const GLchar *const*uniformNames, GLuint *uniformIndices) {
    ...
    // ARB_uniform_buffer_object / GL 4.6 sec. 7.6.1: "The error INVALID_VALUE
    // is generated by GetUniformIndices and GetActiveUniformsiv if
    // <uniformCount> is less than zero", and "If an error occurs, nothing is
    // written to <uniformIndices>". Previously a negative count simply skipped
    // the loop and returned GL_NO_ERROR.
    if (uniformCount < 0) {
        context->pushError(GL_INVALID_VALUE);
        return;
    }
    ...
    for (GLsizei i = 0; i < uniformCount; ++i) {
        ...
        switch (pname) {
            ...
            case GL_UNIFORM_ARRAY_STRIDE: {
                ...
                // GL 4.6 Table 7.6: ARRAY_STRIDE is "-1 ... if the uniform
                // is not an array, or is not declared inside a uniform block"
                // -- the two cases report different values. A uniform in the
                // default (non-block) partition reports -1; a non-array
                // uniform that *is* inside a block reports 0.
                if (u.blockIndex < 0) {
                    params[i] = -1;
                    break;
                }
            ...
            case GL_UNIFORM_MATRIX_STRIDE: {
                ...
                // GL 4.6 Table 7.6: MATRIX_STRIDE is -1 for uniforms that
                // are not matrices *or* are not declared inside a uniform
                // block; 0 only applies to a non-matrix member of a block.
                // Atomic counters keep the ARB_shader_atomic_counters answer
                // of 0 rather than falling into the default-partition case.
                if (u.blockIndex < 0 && u.type != GL_UNSIGNED_INT_ATOMIC_COUNTER) {
                    params[i] = -1;
                    break;
                }
...
static void APIENTRY glGetActiveUniformName(GLuint program, GLuint uniformIndex, GLsizei bufSize, GLsizei *length, GLchar *uniformName) {
    ...
    // GL 4.6 sec. 7.6.1: INVALID_VALUE if <bufSize> is negative. Nothing is
    // written to <uniformName> when an error is generated.
    if (bufSize < 0) {
        context->pushError(GL_INVALID_VALUE);
        return;
    }
...
static GLuint APIENTRY glGetUniformBlockIndex(GLuint program, const GLchar *uniformBlockName) {
    ...
    GLProgramObject* obj = context->objects().programs().get(program);
    if (obj == nullptr) {
        // GL 4.6 sec. 7.6.2 / ARB_uniform_buffer_object: "The error
        // INVALID_VALUE is generated by ... GetUniformBlockIndex ... if
        // <program> is not a value generated by GL." We returned
        // GL_INVALID_INDEX silently with GL_NO_ERROR.
        context->pushError(GL_INVALID_VALUE);
...
static void APIENTRY glGetActiveUniformBlockName(GLuint program, GLuint uniformBlockIndex, GLsizei bufSize, GLsizei *length, GLchar *uniformBlockName) {
    ...
    // GL 4.6 sec. 7.6.2: INVALID_VALUE if <bufSize> is negative. Nothing is
    // written to <uniformBlockName> when an error is generated.
    if (bufSize < 0) {
        context->pushError(GL_INVALID_VALUE);
        return;
    }
...
static bool appglIsTexImage2DMultisampleTarget(GLenum target) {
    return target == GL_TEXTURE_2D_MULTISAMPLE ||
           target == GL_PROXY_TEXTURE_2D_MULTISAMPLE;
}

static bool appglIsTexImage3DMultisampleTarget(GLenum target) {
...
static void APIENTRY glTexImage2DMultisample(GLenum target, GLsizei samples, GLenum internalformat, GLsizei width, GLsizei height, GLboolean fixedsamplelocations) {
    ...
    auto* context = currentContextOrNull();
    if (context == nullptr) return;
    if (!appglIsTexImage2DMultisampleTarget(target)) {
        context->pushError(GL_INVALID_ENUM);
        return;
    }
    if (samples < 1) {
    ...
    if (stored && texName != 0 && target == GL_TEXTURE_2D_MULTISAMPLE) {
...
static void APIENTRY glTexImage3DMultisample(GLenum target, GLsizei samples, GLenum internalformat, GLsizei width, GLsizei height, GLsizei depth, GLboolean fixedsamplelocations) {
    ...
    auto* context = currentContextOrNull();
    if (context == nullptr) return;
    if (!appglIsTexImage3DMultisampleTarget(target)) {
        context->pushError(GL_INVALID_ENUM);
        return;
    }
    if (samples < 1) {
    ...
    GLuint texName = context->state().boundTexture(target);
    if (stored && texName != 0 && target == GL_TEXTURE_2D_MULTISAMPLE_ARRAY) {
        if (auto* texObj = context->objects().textures().get(texName)) {
            texObj->desc.immutable = false;
        }
    }
